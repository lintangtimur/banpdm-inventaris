...
@section('content')
    <div class="col-12">
        ...
        <form class="card" method="post" action="{{ route('stock.store') }}">
        @csrf
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row row-cols-2 g-4">
                    ...
                    <input type="text" placeholder="Nama pengambil" name="source" value="{{ old('source') }}" class="form-control @error("source") is-invalid @enderror">
                    @error('source')
                    ...
                    <input type="text" placeholder="masukkan jumlah" name="quantity" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity') }}">
                    @error('quantity')
                        <div class="invalid-feedback">{{ $message }}</div>    
                    @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input class="form-control @error("date") is-invalid @enderror" placeholder="Select a date" id="datepicker-trx" value="{{ old('date') }}" name="date">
                        @error("date")
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div>
                    ...
                    <select class="form-select @error('inout') is-invalid @enderror" name="inout">
                        ...
                        @foreach ($transactionsType as $key => $item)
                            <option value="{{ $key }}" {{ old('inout') == $key ? 'selected' : '' }}>
                                {{ $item }}
                            </option>
                        @endforeach
